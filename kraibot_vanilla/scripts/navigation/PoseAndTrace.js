...
ROS2D.PoseAndTrace = function(options) {
	...
	this.withTrace = options.withTrace || false; // fixed true
 	this.maxTraceLength = options.maxTraceLength || 100;
	var traceColor = options.traceColor || createjs.Graphics.getRGB(0, 100, 0, 0.50);
	var traceSize = options.traceSize || 1.5;
	var robotColor = options.robotColor || createjs.Graphics.getRGB(255, 0, 0, 0.50);
	var robotSize = options.robotSize || 7;
...
ROS2D.PoseAndTrace.prototype.updatePose = function(pose) {
	...
	this.robotMarker.x = pose.pose.pose.position.x;  //fixed position.x
	this.robotMarker.y = -pose.pose.pose.position.y; //fixed position.y

	// this.robotMarker.x = pose.position.x;  //fixed position.x
	// this.robotMarker.y = -pose.position.y; //fixed position.y

	this.robotMarker.rotation = this.stage.rosQuaternionToGlobalTheta(pose.pose.pose.orientation); //fixed pose.orientation
	// this.robotMarker.rotation = this.stage.rosQuaternionToGlobalTheta(pose.orientation); //fixed pose.orientation
	if (this.initScaleSet) {
		this.robotMarker.visible = false; //fix for disable robotmark
	}
	
	// Draw trace
	if (this.withTrace === true && this.initScaleSet === true) {
		// Turtlebot3
		this.trace.addPose(pose.pose.pose); //fixed pose
		// this.trace.addPose(pose); //fixed pose
