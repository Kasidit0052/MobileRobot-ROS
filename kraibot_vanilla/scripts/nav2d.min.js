...
NAV2D.Navigator = function (options) {
  ...
  if (withOrientation === false) {
    ...
    var mouseEventHandler = function (event, mouseState) {
      if (mouseState === "down" && onPanZoom === false && initPose === false) {
  ...
  this.cancelGoal = cancel_function();
};

//#################################### Initial Pose Estimator for mobile robot
NAV2D.InitPoseEstimator = function (options) {
  var that = this;
  options = options || {};
  var ros = options.ros;
  var topicName = options.topicName || "/initialpose";
  var topicMsgType = options.topicMsgType || "geometry_msgs/PoseWithCovarianceStamped";
  var withOrientation = options.withOrientation || false;
  var arrowSize = options.arrowSize || 25;
  this.rootObject = options.rootObject || new createjs.Container();

  // add initial pose variable
  var initialPose;

  var initpose_publisher = new ROSLIB.Topic({
    ros : ros,
    name : topicName,
    messageType : topicMsgType
  });

  function sendInitialPose(pose) {
    // init publisher
    console.log(pose);
    var default_poseWithCovarianceStamped = new ROSLIB.Message({
      ...
      pose:
      {
      position: {x: parseFloat(pose.position.x), y: parseFloat(pose.position.y), z: parseFloat(pose.position.z)},
      orientation: { x: parseFloat(pose.orientation.x), y: parseFloat(pose.orientation.y), z: parseFloat(pose.orientation.z), w: parseFloat(pose.orientation.w) },
      },
      covariance: [
    ...
    console.log("send Initial pose");
    console.log(initpose_publisher);
    initpose_publisher.publish(default_poseWithCovarianceStamped);

    // save as a class variable
    that.initialPose = default_poseWithCovarianceStamped;
  }
  ...
  if (withOrientation === false) {
    ...
    var mouseEventHandler = function (event, mouseState) {
      if (mouseState === "down" && onPanZoom === false && initPose === true) {
        // get position when mouse button is pressed down
        position = stage.globalToRos(event.stageX, event.stageY);
        positionVec3 = new ROSLIB.Vector3(position);
        mouseDown = true;
...
NAV2D.OccupancyGridClientNav = function (options) {
  ...
  this.initialpose_publisher = null;
  this.topicName = options.topicName || "/initialpose";
  this.topicMsgType = options.topicName || "geometry_msgs/PoseWithCovarianceStamped";
  ...
  client.on("change", function () {
    ...
    //#########################################add initial pose publisher
    that.initialpose_publisher = new NAV2D.InitPoseEstimator({
      ros: that.ros,
      topicName: that.topicName,
      topicMsgType:that.topicMsgType,
      rootObject: that.rootObject,
      withOrientation: that.withOrientation,
      arrowSize: that.arrowSize,
    });
