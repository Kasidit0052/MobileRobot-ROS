...
ROS2D.PathShape = function(options) {
	...
	if (path !== null && typeof path !== 'undefined') {
		...
		this.graphics.moveTo(path.poses[0].position.x / this.scaleX, path.poses[0].position.y / -this.scaleY);
		for (var i=1; i<path.poses.length; ++i) {
			this.graphics.lineTo(path.poses[i].position.x / this.scaleX, path.poses[i].position.y / -this.scaleY);
		}
